...
import (
	forum "forum/Backend" // Assuming your forum package is in a directory named "Backend"
	"log"
	"net/http"
)
...
func main() {
	...
	http.HandleFunc("/google_login", forum.GoogleLoginHandler)
    http.HandleFunc("/google_callback", forum.GoogleCallbackHandler)
    // Github OAuth
    http.HandleFunc("/github_login", forum.GithubLoginHandler)
    http.HandleFunc("/github_callback", forum.GithubCallbackHandler)
